...
class DiagramAiAgent extends BaseAiService {
  ...
  DiagramAiAgent()
    : super(
        ...
        systemInstruction:
            "${AiConfig.documentAnalysisInstruction} "
        ...
        generationConfig: AiUtils.createJsonConfig(
          AiUtils.createCourseExtractionSchema(),
        ),
      );

  /// Step 1: Import a PDF file (grade sheet) using existing PdfService
  /// Returns File on mobile/desktop, or Map with bytes on web
  Future<dynamic> importPdfFile() async {
    try {
      if (kIsWeb) {
        // On web, use bytes-based approach
        return await PdfService.pickPdfBytes();
      } else {
  ...
  /// Accepts either File (mobile) or Map with bytes (web)
  Future<Map<String, dynamic>> extractCourseDataFromPdf(dynamic pdfData) async {
    try {
      Uint8List pdfBytes;

      // Handle both File and bytes Map
      if (pdfData is File) {
        // Mobile/Desktop: Read from File
        pdfBytes = await pdfData.readAsBytes();
      } else if (pdfData is Map<String, dynamic>) {
        // Web: Extract bytes from Map
        pdfBytes = pdfData['bytes'] as Uint8List;
      } else {
        throw Exception('Invalid PDF data type');
      }
      // Create the specific prompt for course data extraction
      const prompt =
          "Can you create a JSON file where keys are the course IDs and the values are the rest of content about each course? "
  ...
  Future<Map<String, dynamic>?> processGradeSheet() async {
    try {
      // Step 1: Import PDF file (returns File on mobile or Map on web)
      final pdfData = await importPdfFile();
      if (pdfData == null) {
        return null; // User cancelled file selection
      }

      // Step 2 & 3: Extract and save course data
      final courseData = await extractCourseDataFromPdf(pdfData);
  ...
  List<Map<String, dynamic>> getCoursesForApp() {
    if (_extractedCourseData == null ||
        _extractedCourseData!['courses'] == null) {
      return [];
    }
  ...
  Map<String, List<Map<String, dynamic>>> getCoursesBySemester() {
    ...
    final sortedKeys =
        coursesBySemester.keys.toList()..sort(_compareSemesterKeys);
  ...
  List<Map<String, dynamic>> _normalizeCoursesData(List<dynamic> courses) {
    return courses
          ...
          if (semester.contains(' ') && year.isEmpty) {
            ...
            if (parts.length >= 2) {
              ...
              if (parts[0].contains(RegExp(r'\d'))) {
                ...
                semester = parts
                    .sublist(0, parts.length - (year.isEmpty ? 0 : 1))
                    .join(' ');
              }
            }
          }
        ...
        .where((course) => course['courseId'] != 'N/A')
        .toList();
  }
  ...
  Map<String, dynamic> getExtractionSummary() {
    ...
    return {
      ...
      'yearRange':
          uniqueYears.isEmpty
              ? ''
              : '${uniqueYears.first} - ${uniqueYears.last}',
  ...
  Map<String, List<Map<String, dynamic>>> getCoursesByYear() {
    ...
    // Sort years
    final sortedKeys =
        coursesByYear.keys.toList()..sort((a, b) => a.compareTo(b));
